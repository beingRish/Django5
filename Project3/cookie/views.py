...
def getcookie(request):
    ...
    return render(request, 'cookie/getcookie.html', {'pay_id': pay_id})
...
def setsignedcookie(request):
    response = render(request, 'cookie/setsignedcookie.html')
    response.set_signed_cookie('token', 't123456', salt='tk')
    return response


def getsignedcookie(request):
    token = request.get_signed_cookie('token', default="guest_toke123", salt='tk')
    return render(request, 'cookie/getsignedcookie.html', {'token': token})